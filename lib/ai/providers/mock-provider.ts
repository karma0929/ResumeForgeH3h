...
export class MockAIProvider implements AIProvider {
  ...
  async extractJobPosting(input: ExtractJobPostingInput): Promise<ExtractedJobPostingOutput> {
    const analysis = analyzeJobDescription(input.jobPostingText);
    const defaultBrief = createDefaultTargetRoleBriefData();
    const domainParts = (() => {
      try {
        return new URL(input.sourceUrl).hostname.split(".");
      } catch {
        return [];
      }
    })();
    const guessedCompany = domainParts.length >= 2 ? domainParts[domainParts.length - 2] : "";

    return {
      sourceUrl: input.sourceUrl,
      company: guessedCompany ? guessedCompany[0].toUpperCase() + guessedCompany.slice(1) : "",
      role: "",
      location: "",
      cleanedJobDescription: input.jobPostingText.slice(0, 12000),
      briefData: {
        ...defaultBrief,
        topRequiredSkills: analysis.mustHaves,
        emphasizeKeywords: analysis.keywords.slice(0, 10),
        responsibilitiesSummary: analysis.summary,
      },
    };
  }
}
